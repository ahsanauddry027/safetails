...
const AdminPostsPage = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setPosts((prev) =>
        prev.filter((post) => post._id !== deleteConfirm.postId)
      );
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {posts.map((post) => (
                    <tr
                      key={post._id}
                      className="hover:bg-gray-50 transition-all duration-200 hover:shadow-sm border-l-4 border-l-transparent hover:border-l-blue-500"
                    >
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          <div className="flex space-x-2">
                            <span
                              className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getPostTypeBadgeClass(post.postType)}`}
                            >
                              ...
                              {post.location.coordinates &&
                                post.location.coordinates.length === 2 && (
                                  <div className="text-xs text-gray-400 mt-1">
                                    Coordinates:{" "}
                                    {post.location.coordinates[0].toFixed(6)},{" "}
                                    {post.location.coordinates[1].toFixed(6)}
                                  </div>
                                )}
