  ...
  switch (method) {
    ...
    case "DELETE":
      try {
        ...
        // Delete post
        const result = await PetPostController.deletePost(id as string, userId);
    ...
    case "PATCH":
      try {
        ...
        if (action === "resolve") {
          // Resolve post
          const resolvedPost = await PetPostController.resolvePost(id as string, userId);
          ...
          const updatedPost = await PetPostController.addComment(id as string, userId, text);
          res.status(200).json({ success: true, data: updatedPost });
        } else {
          res.status(400).json({ success: false, message: "Invalid action" });
        }
      } catch (error: any) {
        if (error.message === "Not authorized to resolve this post") {
          res.status(403).json({ success: false, message: error.message });
        } else if (error.message === "Post not found" || error.message === "User not found") {
          res.status(404).json({ success: false, message: error.message });
        } else {
          res.status(500).json({ success: false, message: error.message });
        }
