// pages/api/admin/adoption/[id].ts
...
export default async function handler(
  ...
  try {
    ...
    if (!id) {
      return res.status(400).json({ error: "Adoption post ID is required" });
    }

    // Check if adoption post exists
    const adoptionPost = await Adoption.findById(id);
    if (!adoptionPost) {
      return res
        .status(404)
        .json({ error: "Adoption post not found or already deleted" });
    }
