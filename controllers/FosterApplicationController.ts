...
import FosterApplication, {
  IFosterApplication,
} from "@/models/FosterApplication";
...
async function getFosterApplications(
  req: NextApiRequest,
  res: NextApiResponse
) {
  try {
    ...
    // Build filter object
    const filter: import("mongoose").FilterQuery<IFosterApplication> = {};
    ...
    return res
      .status(500)
      .json({ message: "Failed to fetch foster applications" });
  }
}

// Create a new foster application
async function createFosterApplication(
  req: NextApiRequest,
  res: NextApiResponse
) {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Foster request is not accepting applications" });
    }
    ...
    if (existingApplication) {
      return res
        .status(400)
        .json({ message: "You have already applied for this foster request" });
    }
    ...
    await fosterApplication.populate(
      "fosterRequestId",
      "petName petType petBreed status"
    );
    ...
    return res
      .status(500)
      .json({ message: "Failed to submit foster application" });
  }
}

// Update foster application (for admin approval/rejection)
async function updateFosterApplication(
  req: NextApiRequest,
  res: NextApiResponse
) {
  try {
    ...
    if (status === "approved") {
      await FosterRequest.findByIdAndUpdate(
        updatedApplication.fosterRequestId,
        {
          status: "matched",
          fosterParent: {
            userId: (
              updatedApplication.applicantId as unknown as { _id: string }
            )._id,
