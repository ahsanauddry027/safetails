...
async function getAdoptionApplications(
  req: NextApiRequest,
  res: NextApiResponse
) {
  try {
...
async function createAdoptionApplication(
  req: NextApiRequest,
  res: NextApiResponse
) {
  try {
    ...
    if (adoption.status !== "available") {
      return res
        .status(400)
        .json({ message: "Adoption listing is not accepting applications" });
    }
    ...
    if (existingApplication) {
      return res
        .status(400)
        .json({ message: "You have already applied for this adoption" });
    }
...
async function updateAdoptionApplication(
  req: NextApiRequest,
  res: NextApiResponse
) {
  try {
